...
use crate::tables::{mana_source, SrcMode};
use crate::win;
use rand::Rng;
...
pub fn tap_out(state: &GameState) -> GameState {
    ...
    for i in 0..s.battlefield.len() {
        if let Some((mode, produced)) = mana_source(s.battlefield[i].effective_name()) {
...
pub fn deploy_engine_perms(s: &mut GameState, reg: &Registry) -> Vec<String> {
    ...
    loop {
        ...
        for nm in &cands {
            ...
            // a fresh tap source taps for mana now
...
impl ProbabilisticPlanner {
    pub fn best_line<R: Rng + ?Sized>(
        ...
        if let Some(bb) = breach_base {
            ...
            let enough_fuel = loops::gy_fuel(&bb, None) >= 4 || bb.has_permanent("Lion's Eye Diamond");
