...
pub fn exile_fuel(s: &mut GameState, k: i64, protect: &[String]) {
    let mut removed = 0i64;
    let mut i = 0;
    while i < s.graveyard.len() && removed < k {
        if protect.contains(&s.graveyard[i]) {
            i += 1;
            continue;
        }
    ...
    while removed < k && !s.graveyard.is_empty() {
        s.graveyard.pop();
        removed += 1;
    }
}
...
pub fn rollout_from<R: Rng + ?Sized>(
    ...
    for _ in 0..max_steps {
        ...
        for (card, source) in &cands {
            if let Some((ns, _)) = do_cast(&s, reg, card, source, rng, payoffs) {
                nxt = Some(ns);
                break;
            }
        }
...
pub fn rollout_estimate<R: Rng + ?Sized>(
    ...
    for _ in 0..n_sims {
        ...
        if won.is_none() {
            ...
            match ns {
                Some(ns) => {
                    won = rollout_from(ns, reg, payoffs, need_life, rng, max_steps - 1);
                }
                None => {
                    ran += 1;
                    if let Some(need) = need {
...
pub fn prove_go_off<R: Rng + ?Sized>(
    ...
    match do_cast(&s, reg, card, &src, rng, payoffs) {
        Some((ns, _)) => rollout_from(ns, reg, payoffs, need_life, rng, max_steps - 1).is_some(),
        None => false,
    }
}
