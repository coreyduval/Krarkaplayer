...
fn run_effect<R: Rng + ?Sized>(
    ...
    match card {
        ...
        "Frantic Search" => {
            let m = untap_mana(state, reg, "Frantic Search") * n;
            state.mana.add("*", m);
            for _ in 0..n {
        ...
        "Snap" => {
            let m = untap_mana(state, reg, "Snap") * n;
            state.mana.add("*", m);
            true
        }
        "Gamble" => {
            for _ in 0..n {
                ...
                let fetched = wishlist::best(state, reg, &state.library.clone(), 1, true)[0].clone();
                if let Some(pos) = state.library.iter().position(|c| *c == fetched) {
                    state.library.remove(pos);
                }
